...
interface EditEntryModalProps {
  entry: AdminDictionaryEntry;
  onClose: () => void;
  onSubmit: (entryData: Partial<AdminDictionaryEntry>) => Promise<AdminOperationResult>;
}

export default function EditEntryModal({ entry, onClose, onSubmit }: EditEntryModalProps) {
  const [formData, setFormData] = useState({
    word: entry.word,
    definition: entry.meanings[0]?.definitions[0]?.definition || '',
    partOfSpeech: entry.meanings[0]?.partOfSpeech || '',
    status: entry.status || 'published' as 'published' | 'draft' | 'archived'
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create update data in the expected format
      const updateData: Partial<AdminDictionaryEntry> = {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          >
            <SvgIcon className="h-6 w-6" variant="default" icon="Plus" />
          </button>
        </div>
