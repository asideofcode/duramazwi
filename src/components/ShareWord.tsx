...
export default function ShareWord({ word, definition, className = '' }: ShareWordProps) {
  const [copied, setCopied] = useState(false);
  
  const shareUrl = typeof window !== 'undefined' 
    ? window.location.href 
    : `https://shonadictionary.com/word/${encodeURIComponent(word)}`;
  
  const shareText = `Check out the meaning of "${word}" in Shona: ${definition}`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Meaning of ${word} in Shona`,
          text: shareText,
          url: shareUrl,
        });
      } catch (err) {
